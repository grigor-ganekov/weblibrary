				...
				<div class="wrapper style2">
					<div class="inner">
						<div class="container">
									...
									<article>	
                                          <form action="search">
                                              <input type="text" name="search" value="<%=result.getSearchField() %>" style="width: 50%" />
                                                      <select name="type" style="width: 15%" >
                                                      <%if (result.getMethod().equals("author")){ %>
			                                          <option value="headline">headline</option>
			                                          <option value="author" selected="selected">author</option>
			                                          <%}else{ %>
			                                          <option value="headline"selected="selected">headline</option>
                                     ...
                                     <%if (result.getPages()>1){ %>  
                                      <form id="searchf" action="goto">    
                                       <input type="hidden" name="search" value="<%=result.getSearchField()%>" />
                                       <input type="hidden" name="type" value="<%=result.getMethod()%>" />
                                       <script>
											function myFunction() {
					...
					<tbody>
						...
						<tr><td><a href="book?id=<%=aBook.getId()%>" style="text-decoration: none"><%=aBook.getHeadline()%></a></td><td><a href="book?id=<%=aBook.getId()%>" style="text-decoration: none"><%=aBook.getAuthor()%></a></td><td><a href="book?id=<%=aBook.getId()%>" style="text-decoration: none"><%=aBook.getPublisher()%></a></td><td><a href="book?id=<%=aBook.getId()%>" style="text-decoration: none"><%=aBook.getYearOfPublishing()%></a></td></tr>
